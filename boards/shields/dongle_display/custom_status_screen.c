...
lv_obj_t *zmk_display_status_screen() {
    ...
    lv_obj_set_style_transform_angle(zmk_widget_output_status_obj(&output_status_widget), 900, 0); // 90 degrees * 10
    
    zmk_widget_bongo_cat_init(&bongo_cat_widget, screen);
    lv_obj_align(zmk_widget_bongo_cat_obj(&bongo_cat_widget), LV_ALIGN_CENTER, 0, 0);
    lv_obj_set_style_transform_angle(zmk_widget_bongo_cat_obj(&bongo_cat_widget), 900, 0);

#if IS_ENABLED(CONFIG_ZMK_HID_INDICATORS)
    zmk_widget_hid_indicators_init(&hid_indicators_widget, screen);
    lv_obj_align(zmk_widget_hid_indicators_obj(&hid_indicators_widget), LV_ALIGN_BOTTOM_RIGHT, -2, -2);
    lv_obj_set_style_transform_angle(zmk_widget_hid_indicators_obj(&hid_indicators_widget), 900, 0);
#endif
    ...
    lv_obj_set_style_transform_angle(zmk_widget_layer_stats_obj(&layer_status_widget), 900, 0);

    zmk_widget_dongle_battery_status_init(&dongle_battery_status_widget, screen);
    lv_obj_align(zmk_widget_dongle_battery_status_obj(&dongle_battery_status_widget), LV_ALIGN_TOP_RIGHT, 0, 0);
    lv_obj_set_style_transform_angle(zmk_widget_dongle_battery_status_obj(&dongle_battery_status_widget), 900, 0);
